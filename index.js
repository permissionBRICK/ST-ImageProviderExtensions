...
/**
 * Keys that are NOT part of a settings preset snapshot.
 * A preset captures backend/connection params (source, model, sampler, dimensions, etc.)
 * but not the prompt library, styles, custom entries, or UI prefs.
 * @type {string[]}
 */
...
/**
 * Creates a deep clone snapshot of the settings keys that belong to a preset.
 * @returns {object} Snapshot of the included settings keys.
 */
function snapshotSdSettings() {
    const snapshot = {};
    for (const key of Object.keys(extension_settings.sd)) {
        if (PRESET_EXCLUDE_KEYS.includes(key)) {
            continue;
        }
        snapshot[key] = structuredClone(extension_settings.sd[key]);
    }
    return snapshot;
}

/**
 * Applies a settings snapshot to the live settings object (in-memory only; does NOT touch the DOM).
 * @param {object} snapshot Snapshot previously created by snapshotSdSettings().
 */
function applySdSettingsSnapshot(snapshot) {
    ...
    for (const key of Object.keys(snapshot)) {
        ...
        extension_settings.sd[key] = structuredClone(snapshot[key]);
    }
}

/**
 * Checks whether a preset is configured (a non-null object with at least one key).
 * @param {object} preset Preset to check.
 * @returns {boolean} True if the preset is configured.
 */
...
/**
 * Refreshes all settings UI controls to reflect the current extension_settings.sd values.
 * Used after loading a settings preset.
 * @returns {Promise<void>}
 */
async function refreshSettingsUi() {
    // loadSettings() appends to #sd_style without clearing it, so empty it first.
    $('#sd_style').empty();
    await loadSettings();
}
...
async function sendGenerationRequest(generationType, prompt, additionalNegativePrefix, characterName, callback, initiator, signal) {
    ...
    /**
     * Performs a single image generation attempt against the live extension_settings.sd config.
     * Reads settings live so it can be retried after applying a fallback preset.
     * @param {AbortSignal} attemptSignal Abort signal to cancel the request.
     * @returns {Promise<{result: {format: string, data: string}, prefixedPrompt: string}>}
     * @throws {Error} On failure or when the endpoint returns no image data.
     */
    async function attemptImageGeneration(attemptSignal) {
        ...
        switch (extension_settings.sd.source) {
            case sources.extras:
                result = await generateExtrasImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.horde:
                result = await generateHordeImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.vlad:
                result = await generateAutoImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.drawthings:
                result = await generateDrawthingsImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.auto:
                result = await generateAutoImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.sdcpp:
                result = await generateSdcppImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.novel:
                result = await generateNovelImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.openai:
                result = await generateOpenAiImage(prefixedPrompt, attemptSignal);
                break;
            case sources.aimlapi:
                result = await generateAimlapiImage(prefixedPrompt, attemptSignal);
                break;
            case sources.comfy:
                ...
                break;
            case sources.togetherai:
                result = await generateTogetherAIImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.pollinations:
                result = await generatePollinationsImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.stability:
                result = await generateStabilityImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.huggingface:
                result = await generateHuggingFaceImage(prefixedPrompt, attemptSignal);
                break;
            case sources.chutes:
                result = await generateChutesImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.electronhub:
                result = await generateElectronHubImage(prefixedPrompt, attemptSignal);
                break;
            case sources.nanogpt:
                result = await generateNanoGPTImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.bfl:
                result = await generateBflImage(prefixedPrompt, attemptSignal);
                break;
            case sources.falai:
                result = await generateFalaiImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.xai:
                result = await generateXAIImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.google:
                result = await generateGoogleImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
            case sources.zai:
                result = await generateZaiImage(prefixedPrompt, attemptSignal);
                break;
            case sources.openrouter:
                result = await generateOpenRouterImage(prefixedPrompt, attemptSignal);
                break;
            case sources.workersai:
                result = await generateWorkersAIImage(prefixedPrompt, negativePrompt, attemptSignal);
                break;
        }
